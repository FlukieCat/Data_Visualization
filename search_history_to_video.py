            ...
            try:
                ...
                for w in search_string.split(" "):
                    if w.lower() not in stopwords:
                        c.execute("INSERT INTO words (unix, word) VALUES (?, ?)", (d, w.lower()))
            except Exception as e:
                print(str(e))
    conn.commit()

def words_to_image(conn):
    DAY = 86400
    MONTH = 30 * DAY
    YEAR = 365 * DAY
...
def main():
    ...
    conn = sqlite3.connect('search_key_words.db')
    make_table(conn)
    populate_db(HTML_FILE, conn)
    words_to_image(conn)
    conn.close()
